...
//     label: 'Planning Date Created', bg: 'secondary.main', icon: <IconCalendarEvent size={22} />,
//     to: (n) => ROUTES['project-planning-date-created'](String(n.projectId)),
//   },
//   // 'project-planning-date-created': {
//   //   label: 'Planning Date Created',
//   //   bg: 'secondary.main',
//   //   icon: <IconCalendarEvent size={22} />,
//   //   to: ROUTES['project-planning-date-created'], 
//   // },
...
//   // --- دومین مودال: انتخاب آیتم‌های دارای ID ---
...
//       const list = (s?.notis ?? []).filter(n => !n.read);
//       debugger
...
//       let resp;
//       if (notifIds?.length) {
//         // وقتی id خود نوتیف‌ها رو داری: تک‌به‌تک
//         const calls = notifIds.map((id) =>
//           axios.put(`${base}set-system-notification-read/${encodeURIComponent(id)}/${encodeURIComponent(type)}/${encodeURIComponent(role)}`,
...
//           .filter((x: any) => Number(x.recordStatus) === 1)
//           .map((x: any) => String(x.id));
//         // حذف از UI
//         setItems(prev => prev.filter(n => !updatedIds.includes(String(n.id))));
//         return updatedIds;
//       } else {
//         // حالت بدون id: براساس type/role
//         resp = await axios.put(
//           `${base}set-system-notification-read/${encodeURIComponent(type)}/${encodeURIComponent(role)}`,
//           null, { headers: { Authorization: `Bearer ${authToken}` } }
//         );
//         const arr: MarkReadResp = Array.isArray(resp.data?.data) ? resp.data.data : [];
//         const updatedIds = arr
//           .filter(x => Number(x.recordStatus) === 1)
//           .map(x => String(x.id));
//         setItems(prev => prev.filter(n => !updatedIds.includes(String(n.id))));
//         return updatedIds;
//       }
...
//     // ↓↓↓ تغییر اصلی: همیشه مودال را باز کن
//     setSelectType(type);
//     setSelectEntityIds(entityIds);
//     setSelectOpen(true);
//     setAnchorEl2(null);    // منو را ببند تا تمرکز روی مودال باشد

//     // در همان تیک event loop fetch کن (UI بلاک نشود)
//     const needFetch = () => {
//       if (need === 'warehouse') fetchWarehouses();
//       if (need === 'store') fetchStores();
//       if (need === 'project') fetchProjects();
//     };
//     setTimeout(needFetch, 0);
//   };
...
//   // فیلتر آیتم‌های مودال انتخاب بر اساس entityIds
...
//                         <Tooltip title={meta.label + (sample?.projectTitle ? ` — ${sample.projectTitle}` : '')}>
//                           <Typography variant="subtitle2" color="text.primary" fontWeight={600} noWrap sx={{ width: 240 }}>
//                             {meta.label}{sample?.projectTitle ? ` — ${sample.projectTitle}` : ''}
//                           </Typography>
//                         </Tooltip>
//                       </Box>
...
//                                       <Typography variant="subtitle2" noWrap>{`${w.name} (${w.code})`}</Typography>
//                                     </Stack>
//                                   }
//                                   secondary={w.address}
//                                 />
//                               </Box>
//                               {cnt > 0 && <Chip sx={{ marginLeft: "10px" }} size="small" color='success' label={cnt} />}
//                               <Button
//                                 variant="contained"
//                                 size="small"
//                                 onClick={() => goFromSelection(String(w.id), entityNotifIds)}
//                               >
//                                 Görüntüle
//                               </Button>
//                             </Stack>
...
//                                   primary={
//                                     <Stack direction="row" alignItems="center" spacing={1}>
//                                       <Typography variant="subtitle2" noWrap>{`${s.name} (${s.code})`}</Typography>
//                                     </Stack>
//                                   }
//                                   secondary={s.address}
//                                 />
...
//                                 variant="contained"
//                                 size="small"
//                                 onClick={() => goFromSelection(String(s.id), entityNotifIds)}
//                               >
//                                 Görüntüle
//                               </Button>
//                             </Stack>
...
//                                 variant="contained"
//                                 size="small"
//                                 onClick={() => goFromSelection(String(p.id), entityNotifIds)}
//                               >
//                                 Görüntüle
//                               </Button>
//                             </Stack>
...
const Notifications = () => {
  ...
  //     const calls = notifIds.map((id) =>
  //       axios.put(
  //         `${base}set-system-notification-read/${encodeURIComponent(id)}/${encodeURIComponent(type)}/${encodeURIComponent(role)}`,
  //         null, { headers: { Authorization: `Bearer ${authToken}` } }
  //       )
  //     );
  //     const results = await Promise.all(calls);
  ...
  //     );
  //     const arr: MarkReadResp = Array.isArray(resp.data?.data) ? resp.data.data : [];
  //     const updatedIds = arr.filter(x => Number(x.recordStatus) === 1).map(x => String(x.id));
  //     setItems(prev => prev.filter(n => !updatedIds.includes(String(n.id))));
  //     return updatedIds;
  //   }
  // } catch (e) {
  ...
  const markNotisAsRead = async (type: NotifyType, notifIds?: string[]): Promise<string[]> => {
    ...
    debugger
    try {
      // اگر شناسه‌هایی برای نوتیفیکیشن‌ها داده شده باشد
      if (notifIds?.length) {
        const calls = notifIds.map((id) =>
          axios.put(
