...
interface ReceiptDetailType {
    id: string;
    quantity: string;         // رسید شده در این سند
    description: string;
    item: ItemType;
...
interface SendedReceiptType {
    ...
    createAt: string;
    recordStatus: number;
    isEnd: boolean | null;     // 👈 برای Sonlandırılmış
...
const ListReceiptsDestructionSendedFromStore = () => {
    ...
    // New states for end/restore flow
    ...
    const [hiddenWarehouseIds, setHiddenWarehouseIds] = useState<Set<number>>(new Set()); // depo های پنهان در کمبو
    ...
    const loadReceiptDetailsFromDispatchInfo = useCallback(() => {
        if (dispatchDetailsInfo.length === 0) { setReceiptDetails([]); return; }
    ...
    const handleLoadDispatchDetails = () => {
        if (!selectedWarehouseId) return showAlert('Lütfen Giriş Depo alanını doldurun.', 'warning');
        if (dispatchDetailsInfo.length === 0) return showAlert('Sevk detayı bulunamadı. Lütfen filtreleri kontrol edin.', 'warning');
        loadReceiptDetailsFromDispatchInfo();
    };

    // FETCH dispatch details for destruction/return flow
    const fetchDispatchDetails = useCallback(async (warehouseId: number | string) => {
        ...
        try {
            const response = await axios.get<ApiResponse<any[]>>(
                `${server.baseurl}${server.warehouse}get-store-dispatches-return-by-center-id/${warehouseId}`,
                { headers: { Authorization: `Bearer ${authToken}` } }
            );
            if (response.data.httpStatusCode === 200 && Array.isArray(response.data.data)) {
                const formatted: DispatchDetailInfo[] = response.data.data.flatMap((dispatch: any) =>
                ...
                setDispatchDetailsInfo(formatted);
                return formatted;
            } else {
                setDispatchDetailsInfo([]);
                showAlert('Sevk detayları yüklenirken bir hata oluştu.', 'warning'); return [];
            }
        } catch {
            setDispatchDetailsInfo([]);
            showAlert('Sevk detayları yüklenirken bir hata oluştu.', 'error'); return [];
    ...
    }, [authToken, fetchWarehouses, navigate, showAlert]);
    ...
    // form validity
    useEffect(() => {
        const ok = !!selectedWarehouseId && !!docDate && receiptDetails.length > 0 &&
    ...
    const resetFormAndState = () => {
        ...
        setEditingCode(null);
        setDispatchDetailsInfo([]);
        setDocDateError(false); setWarehouseIdError(false); setReceiptDetailsError(false);
    ...
    const insertReceipt = async () => {
        ...
        try {
            ...
            if (res.data.httpStatusCode === 201) {
                ...
                // لیست را تازه بگیر و رسید جدید را پیدا کن
                const receiptsRes = await axios.get<ApiResponse<SendedReceiptType[]>>(
                    server.baseurl + server.warehouse + `get-Receipt-destruction-sended-from-store-to-warehouse`,
                    { headers: { "Authorization": `Bearer ${authToken}` } }
                );
                if (receiptsRes.data?.httpStatusCode === 200) {
                    ...
                    if (newest) {
                        ...
                        setOpenIsEndModal(true); // سوال Sonlandır
                    }
    ...
    const handleEditClick = async () => {
        ...
        try {
            ...
            if (!destinationWarehouseId) { showAlert('Düzenleme için gereken depo bilgisi eksik.', 'error'); setLoadingData(false); return; }

            const fetched = await fetchDispatchDetails(Number(destinationWarehouseId));
            const map = new Map<number, DispatchDetailInfo>();
            fetched.forEach((info: any) => map.set(Number(info.id), info));
            ...
            const formatted: FormReceiptDetail[] = (selectedRowForMenu.receiptDetails || []).map(d => {
                ...
                const info = map.get(sddId);

                const maxQty = info ? Number(info.quantity) : (sdd?.quantity ? Number(sdd.quantity) : Number(d.quantity));
                const dispatchId = info ? Number(info.dispatchId) : (sdd?.storeDispatchHeaders?.id ? Number(sdd.storeDispatchHeaders.id) : 0);
                const dispatchCode = info ? info.dispatchCode : (sdd?.storeDispatchHeaders?.code || 'N/A');
    ...
    return (
        <Box mt={2}>
                        ...
                        <Grid item xs={12} sm={6}>
                            ...
                            <Autocomplete
                                ...
                                onChange={async (_, newValue) => {
                                    ...
                                    setReceiptDetails([]);
                                    if (newId) {
                                        const details = await fetchDispatchDetails(newId);
                                        if (!editingId && details.length === 0) showAlert('Bu depoya teslim edilecek aktif sevk detayı bulunamadı.', 'warning');
                                    } else { setDispatchDetailsInfo([]); setReceiptDetails([]); }
                    ...
                    <Box mt={4}>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2} gap={1} flexWrap="wrap">
                            ...
                            <Stack direction="row" spacing={1} alignItems="center">
                                <Button variant="outlined" startIcon={<IconPlus />} onClick={handleLoadDispatchDetails} disabled={!selectedWarehouseId || loadingData || receiptDetails.length > 0}>
                        ...
                        {loadingData && selectedWarehouseId ? (
                            ...
                            <Grid container spacing={2}>
                                {receiptDetails.length === 0 && selectedWarehouseId && !editingId ? (
                                    <Grid item xs={12}><Alert severity="info">Yüklenecek sevk detayı bulunamadı veya butona basmadınız.</Alert></Grid>
                                ) : (
            ...
            {/* 👇 Modal: Sonlandırılmış فاکتورها + Geri Al */}
