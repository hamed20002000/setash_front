// src/views/warehouses/DeleteBetweenReceipt.tsx
...
const DeleteBetweenReceipt = ({ openModal, receiptIdToDelete, receiptCodeToDelete, onClose, onDeleteSuccess, showAlert }: Props) => {
    ...
    const handleDeleteReceipt = async () => {
        ...
        try {
            const response = await axios.delete(
                `${server.baseurl}${server.warehouse}delete-between-receipt/${receiptIdToDelete}`,
                {
                    headers: {
            ...
            if (e.response && e.response.status === 500) {
                showAlert('Bu fiş şu anda başka bir yerde kullanıldığı için silinemez.', 'error');
            } else if (e.response && e.response.status === 401) {
                localStorage.removeItem('authToken');
                showAlert('Oturum süreniz doldu, lütfen tekrar giriş yapın.', 'error');
                navigate("/");
            } else {
                const errorMessage = e.response?.data?.message || 'Fiş silinirken beklenmeyen bir hata oluştu, lütfen tekrar deneyin.';
...
export default DeleteBetweenReceipt;