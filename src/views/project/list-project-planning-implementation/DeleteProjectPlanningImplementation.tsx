// DeleteProjectPlanningImplementation.tsx
...
type Props = {
    openModal: boolean;
    implementationIdToDelete: number | null; // ID پیاده‌سازی برای حذف
    onClose: () => void;
    onDeleteSuccess: () => void; // تابعی برای رفرش کردن لیست اصلی
    showAlert: (message: string, severity: 'success' | 'error' | 'warning' | 'info') => void;
};

const DeleteProjectPlanningImplementation = ({ openModal, implementationIdToDelete, onClose, onDeleteSuccess, showAlert }: Props) => {
    ...
    const handleDeleteImplementation = async () => {
        ...
        try {
            ...
            if (response.data.httpStatusCode === 200) {
                ...
                showAlert(response.data.message || 'Uygulama kaydı silinirken bir hata oluştu.', 'error');
                onClose();
            }
        } catch (e: any) {
            if (e.response && e.response.status === 500) {
                ...
                const errorMessage = e.response?.data?.message || 'Uygulama kaydı silinirken beklenmeyen bir hata oluştu, lütfen tekrar deneyin.';
    ...
    return (
        <Dialog
            ...
            <DialogTitle id="alert-dialog-title">
                {"Bu proje uygulama kaydını silmek istediğinizden emin misiniz?"}
            </DialogTitle>
            <DialogContent>
