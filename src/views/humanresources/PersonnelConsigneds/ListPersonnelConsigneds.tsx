...
//                 showAlert('Oturum süreniz doldu, lütfen tekrar giriş yapın.', 'error'); navigate("/");
//             }
//             else showAlert(e.response?.data?.message || 'Mal listesi yüklenirken bir hata oluştu.', 'error');
//         }
//     }, [navigate]);

//     // 🆕 تابع واکشی برای حالت IADE
...
//             });
//             if (res.data.httpStatusCode === 200) {
//                 // فرض می‌کنیم داده‌های برگشتی همان ConsignmentType هستند
...
//     // 🆕 لیست کمکی برای یافتن parentId در حالت IADE
...
//         const finalParentId = isReturning ? getParentIdForReturn(selectedConsignmentId) : 0; // در حالت IADE، parentId باید id رکورد واگذاری باشد
...
//             parentId: Number(finalParentId), // 🆕 از finalParentId استفاده می‌کنیم
//             returnDate: isAssignmentMode ? null : (returnDate ? format(returnDate, 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd')),
//         };
//         if (isEditing) (payload as any).id = Number(editingId);
...
//     // 🆕 Effect برای واکشی کالاهای قابل برگشت هنگام تغییر پرسنل در حالت IADE
//     useEffect(() => {
...
const ListPersonnelConsigneds: React.FC = () => {
    ...
    useEffect(() => {
        // چون consignmentList دیگر لیست کامل نیست، این چک‌ها را ساده‌تر می‌کنیم.
        // fetchPersonnelConsigneds لیست اصلی جدول را می‌آورد و باید همیشه پس از پرسنل لود شود.
