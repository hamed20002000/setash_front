...
//     debugger
//     if (file) {
//       setProfileRawFile(file); // ⬅️ ذخیره فایل خام
...
//   const uploadImage = async (file: File, authToken: string): Promise<string | null> => {
//     const formData = new FormData();
//     formData.append('file', file); // توجه: اینجا فقط یک فایل است، نه 'files'

//     try {
//       const uploadResponse = await axios.post(
//         server.baseurl + server.baseinfo + "upload-files", // ⬅️ API آپلود تک فایل (باید API درست را بدانید)
//         formData,
//         { headers: { 'Content-Type': 'multipart/form-data', 'Authorization': `Bearer ${authToken}` } }
//       );

//       if (uploadResponse.data.httpStatusCode === 201) {
//         // فرض می‌کنیم آدرس کامل عکس در response.data.data.fileUrl برمی‌گردد.
...
//       }
//     } catch (e) {
//       showAlert('Resim yükleme servisine erişilemedi.', 'error');
//       return null;
//     }
//   };
...
//     let finalImageSrc = '';
//     if (profileRawFile) {
//       const url = await uploadImage(profileRawFile, authToken!);
...
//     let finalImageSrc = profileImageUrl; // ⬅️ URL نمایش داده شده را به عنوان مقدار پیش فرض حفظ کن
...
//       // اگر هیچ فایل جدیدی آپلود نشده و عکس پیش‌فرض نمایش داده می‌شود (یعنی کاربر عکس را حذف کرده)
//       finalImageSrc = "";
//     }
...
//                                     label={role.name}
//                                     size="small"
//                                     color={chipColor} // ⬅️ استفاده از success/error
...
const ListUsers = () => {
  ...
  // ⬅️ هندلر مدیریت فایل خام و پیش‌نمایش URL
  const handleImageChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
  ...
  const resetFormAndState = useCallback(() => {
    ...
    setConfirmPassword('');
    setSelectedRoles([]);
    setProfileRawFile(null); // ⬅️ ریست فایل خام
    ...
    try {
      const response = await axios.post(
        server.baseurl + server.user + "create-user",
        {
          ...
          password: password,
          rePassword: confirmPassword,
          imageSrc: finalImageSrc, // ⬅️ ارسال URL نهایی
          roleNames: roleNamesToSend,
        },
        {
          headers: {
  ...
  const editUser = async () => {
    ...
    if (profileRawFile) { // ⬅️ اگر فایل جدید انتخاب شده است
      ...
      // اگر عکس حذف شده و پیش‌فرض نمایش داده می‌شود
      finalImageSrc = "";
    }
