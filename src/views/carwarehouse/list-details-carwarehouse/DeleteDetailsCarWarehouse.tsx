...
const DeleteDetailsCarWarehouse: React.FC<DeleteProps> = ({ openModal, idToDelete, nameToDelete, onClose, onDeleteSuccess, showAlert }) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response.data.httpStatusCode === 200) {
                showAlert('Araç detayı başarıyla silindi!', 'success');
                onDeleteSuccess();
                onClose();
            } else {
            ...
            if (e.response && e.response.status === 500) {
                showAlert('Bu araç detayı başka bir işlemde kullanıldığı için silinemez.', 'error');
    ...
    return (
        <Dialog
            open={openModal}
            onClose={onClose}
            aria-labelledby="delete-car-detail-title"
            aria-describedby="delete-car-detail-desc"
            maxWidth="sm"
            fullWidth
        >
            <DialogTitle id="delete-car-detail-title" sx={{ display: 'flex', alignItems: 'center' }}>
                <Typography variant="h6">Araç Detay Silme Onayı</Typography>
            </DialogTitle>
...
export default DeleteDetailsCarWarehouse;