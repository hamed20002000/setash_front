...
//     return price.toLocaleString('us-US', { style: 'currency', currency: 'TRY', minimumFractionDigits: 0, maximumFractionDigits: 2 });
// };
...
//                 ["Şantiye", report.workhouse_name], [" Depo", report.warehouse_name],
...
//             const tableColumn = ["İhale", "Ürün", "Şantiye", " Depo", "Sipariş Tarihi", "Toplam Miktar", "Demontaj Tutarı"];
...
//             const headerRowData = ["İhale", "Ürün", "Şantiye", " Depo", "Sipariş Tarihi", `Toplam Miktar (${result.data[0]?.unit ?? ''})`, "Demontaj Tutarı"];
...
// const StyledTableCell = styled(MuiTableCell)(({ theme }) => ({
//     fontFamily: 'NotoSans',
//     fontSize: '0.75rem', // کمی فونت را کوچک کردم تا در 50 پیکسل بهتر جا شود
...
//     // تنظیمات شکستن متن برای جلوگیری از بهم ریختگی
//     whiteSpace: 'normal',
//     wordBreak: 'break-word',
//     overflowWrap: 'break-word',
//     padding: '4px', // کاهش پدینگ برای استفاده بهینه از فضای کم
//     textAlign: 'center' // وسط چین کردن برای زیبایی بیشتر در عرض کم
// }));
...
//     MontajPrice: string;
//     DemontajPrice: string; // Added based on requirement
//     item_id: string;
//     item_name: string;
//     unit: string;
//     work_id: string;
...
//     totalDemontaj: number;
//     totalMontaj: number;
//     totalDemontajMontaj: number; // Added to match new structure
//     page: number;
//     pageSize: number;
//     totalPages: number;
//     success: boolean;
...
//     // List of numeric/currency fields
//     const numericFields = [
...
//             const reportTitle = `İhale Akış Detay Raporu: ${report.ihale_title}`;
//             addPdfHeader(doc, reportTitle);

//             const tableColumn = ["Alan", "Değer"];
//             const tableRows = [
//                 ["İhale Başlığı", report.ihale_title],
//                 ["Şantiye", report.workhouse_name],
//                 ["Toplam Miktar", `${parseNumberFromString(report.Quantity)} (${report.unit})`],
//                 ["Demontaj", parseNumberFromString(report.Demontaj)],
//                 ["DemontajMontaj", parseNumberFromString(report.DemontajMontaj)],
//                 ["DemontajMontajPrice", formatPriceDisplay(report.DemontajMontajPrice)],
//                 ["DemontajTutari", formatPriceDisplay(report.DemontajTutari)],
//                 ["MontajPrice", formatPriceDisplay(report.MontajPrice)],
//                 ["DemontajPrice", formatPriceDisplay(report.DemontajPrice)],
//             ];
...
//             doc.save(`İhale_Detay_${report.item_name}_${format(new Date(), 'yyyyMMdd')}.pdf`);
//             showAlert('PDF raporu başarıyla oluşturuldu ve indiriliyor.', 'success');
//         } catch (e: any) { handleApiError(e, 'PDF raporu oluşturulurken bir hata oluştu.'); }
//     };
...
//         try {
//             const workbook = new Excel.Workbook();
//             const sheet = workbook.addWorksheet('İhale Detay', { views: [{ rightToLeft: false }] });

//             const data = [
//                 ['İhale', report.ihale_title],
//                 ['Şantiye', report.workhouse_name],
//                 ['Toplam Miktar', parseNumberFromString(report.Quantity)],
//                 ['Demontaj', parseNumberFromString(report.Demontaj)],
//                 ['DemontajMontaj', parseNumberFromString(report.DemontajMontaj)],
//                 ['DemontajMontajPrice', parseNumberFromString(report.DemontajMontajPrice)],
//                 ['DemontajTutari', parseNumberFromString(report.DemontajTutari)],
//                 ['MontajPrice', parseNumberFromString(report.MontajPrice)],
//                 ['DemontajPrice', parseNumberFromString(report.DemontajPrice)],
//             ];
...
//                 newRow.eachCell((cell) => { cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }; });
//                 const fieldName = row[0] as string;
//                 if (fieldName.includes('Price') || fieldName.includes('Tutari')) { newRow.getCell(2).numFmt = '$ #,##0.00'; }
//             });

//             sheet.columns[0].width = 30; sheet.columns[1].width = 40;
//             const buffer = await workbook.xlsx.writeBuffer();
//             saveAs(new Blob([buffer]), `İhale_Detay_${report.item_name}_${format(new Date(), 'yyyyMMdd')}.xlsx`);
//             showAlert('Excel raporu başarıyla oluşturuldu ve indiriliyor.', 'success');
//         } catch (e: any) { handleApiError(e, 'Excel raporu oluşturulurken bir hata oluştu.'); }
//     };

//     // ✨ NEW: Fetch All Data + Apply Client Search & SORT for Export
//     const fetchAllFilteredData = useCallback(async () => {
//         if (!authToken) { navigate("/"); return null; }
...
//             const tableColumn = [
//                 "İhale", "Şantiye", "Top. Miktar", "Demontaj", "Dem/Mon", "D+M Fiyat", "Dem Tutarı", "Mon Fiyat", "Dem Fiyat"
//             ];
...
//             doc.save(`İhale_Akış_Genel_${format(new Date(), 'yyyyMMdd_HHmm')}.pdf`);
//             showAlert('Genel PDF raporu başarıyla oluşturuldu ve indiriliyor.', 'success');
//         } catch (e) { handleApiError(e, 'Genel PDF raporu oluşturulurken bir hata oluştu.'); }
//     };
...
//             const headerRowData = [
//                 "İhale", "Şantiye", "Toplam Miktar", "Demontaj", "DemontajMontaj",
//                 "DemontajMontajPrice", "DemontajTutari", "MontajPrice", "DemontajPrice"
//             ];

//             sheet.addRow(["İhale Akış Genel Raporu"]);
//             sheet.mergeCells('A1:I1'); sheet.getRow(1).font = { bold: true, size: 14 };
//             sheet.addRow([`Toplam Kayıt: ${result.data.length}`]);
//             sheet.addRow([]);
...
//                 [3, 4, 5].forEach(idx => newRow.getCell(idx).numFmt = '#,##0.00');
//                 [6, 7, 8, 9].forEach(idx => newRow.getCell(idx).numFmt = '$ #,##0.00');
...
//             const buffer = await workbook.xlsx.writeBuffer();
//             saveAs(new Blob([buffer]), `İhale_Akış_Genel_${format(new Date(), 'yyyyMMdd_HHmm')}.xlsx`);
...
//         { label: 'Şantiye', key: 'workhouse_name' },
//         { label: 'Toplam Miktar', key: 'Quantity' },
//         { label: 'Demontaj', key: 'Demontaj' },
//         { label: 'DemontajMontaj', key: 'DemontajMontaj' },
//         { label: 'DemontajMontajPrice', key: 'DemontajMontajPrice' },
//         { label: 'DemontajTutari', key: 'DemontajTutari' },
//         { label: 'MontajPrice', key: 'MontajPrice' },
//         { label: 'DemontajPrice', key: 'DemontajPrice' },
//         { label: 'İşlemler', key: 'actions' },
//     ];
...
//                 {/* Search Bar & Global Export Buttons */}
//                 <Box sx={{ p: 2, mt: 1 }}>
//                     <Grid container spacing={2} alignItems="center">
//                         <Grid item xs={12} md={6}>
//                             <TextField
//                                 label="Tabloda Ara (İhale, Şantiye...)"
//                                 variant="outlined" fullWidth size="small"
//                                 value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
//                                 InputProps={{ startAdornment: (<InputAdornment position="start"><IconSearch size={20} /></InputAdornment>) }}
//                             />
...
//                                                 onClick={() => handleRequestSort(header.key as keyof TenderFlowReportRowType)}
//                                             >
//                                                 <Typography variant="h6" fontWeight="bold">{header.label}</Typography>
//                                                 {orderBy === header.key ? (
...
//                                         <StyledTableCell>{parseNumberFromString(row.Demontaj)}</StyledTableCell>
//                                         <StyledTableCell>{parseNumberFromString(row.DemontajMontaj)}</StyledTableCell>
...
//                         <Box sx={{ p: 2, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 3 }}>
//                             <Typography variant="h6" color="secondary">
//                                 Toplam Demontaj: {parseNumberFromString(reportData.totalDemontaj ?? 0)}
//                             </Typography>
//                             <Typography variant="h6" color="success.main">
//                                 Toplam Montaj: {parseNumberFromString(reportData.totalMontaj ?? 0)}
//                             </Typography>
//                             {/* ✨ اضافه شده: جمع دمونتاژ+مونتاژ */}
//                             <Typography variant="h6" color="warning.main">
//                                 Toplam Dem+Mon: {parseNumberFromString(reportData.totalDemontajMontaj ?? 0).toLocaleString()}
//                             </Typography>
//                         </Box>
//                     )}
//                 </>
