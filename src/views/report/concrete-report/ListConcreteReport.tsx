...
// import { visuallyHidden } from '@mui/utils'; // حذف شد تا ارور نده
...
const ListConcreteReport = () => {
    ...
    const handleApiError = useCallback((e: any, defaultMessage: string = 'Bir hata oluştu.') => {
        console.error("API Error Full Detail:", e); // لاگ دقیق خطا
    ...
    const fetchConcreteReportData = useCallback(async () => {
        ...
        // بررسی اینکه آیا آدرس سرور درست لود شده یا نه
        ...
        try {
            const response = await axios.get(
                server.baseurl + server.report + `get-beton-filtered-report-data/`,
                {
                    ...
                    timeout: 10000 // 10 ثانیه تایم اوت برای جلوگیری از گیر کردن
                }
            );
            ...
            setLoadingData(false); // این خط تضمین میکنه لودر همیشه خاموش بشه
        }
    }, [
