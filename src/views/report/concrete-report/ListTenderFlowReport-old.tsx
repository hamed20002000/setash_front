...
const ListTenderFlowReport = () => {
    ...
    const fetchTenderFlowReportData = useCallback(async () => {
        ...
        try {
            const response = await axios.get(
                server.baseurl + server.report + `get-tender-flow-report-for-project-data`,
    ...
    const handleExportPdfAll = () => { // نیازی به async نیست چون دیتا در حافظه موجود است
        ...
        if (!dataToExport || dataToExport.length === 0) {
            showAlert('Rapor indirilemedi: Filtreye uygun veri bulunmamaktadır.', 'warning');
            return;
        }
        ...
        try {
            ...
            autoTable(doc, {
                ...
                foot: [[
                    ...
                    '',
                    '',
                    calculatedTotals.totalDemontaj.toLocaleString('us-US', { minimumFractionDigits: 2 }),
                    calculatedTotals.totalDemontajMontaj.toLocaleString('us-US', { minimumFractionDigits: 2 }),
                    '',
                    '',
                    calculatedTotals.totalMontaj.toLocaleString('us-US', { minimumFractionDigits: 2 }),
                    ''
                ]],
            ...
            doc.save(`Ihale_Akis_Genel_${format(new Date(), 'yyyyMMdd_HHmm')}.pdf`);
            showAlert('Genel PDF raporu başarıyla oluşturuldu.', 'success');
        } catch (e) {
            console.error(e);
            showAlert('Genel PDF raporu oluşturulurken یک خطا رخ داد.', 'error');
        }
    };
    ...
    const handleExportExcelAll = async () => {
        ...
        try {
            ...
            sheet.addRow([
                ...
                "",
                "",
                `Top. Demontaj: ${calculatedTotals.totalDemontaj.toLocaleString()}`,
                `Top. Dem+Mon: ${calculatedTotals.totalDemontajMontaj.toLocaleString()}`,
                "", "", "",
            ...
            console.error(e);
            showAlert('Excel raporu oluşturulurken bir hata oluştu.', 'error');
        }
    };
