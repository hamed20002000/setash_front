...
import ViewStoreBalanceModal from './ViewStoreBalanceModal'
...
const addPdfHeader = (doc: jsPDF, title: string) => {
    ...
    docAny.addImage(Logo, 'PNG', pageWidth - 50, 25, 40, 25);
    doc.setFontSize(14);
...
const ListStores = () => {
    ...
    const [openViewBalanceModal, setOpenViewBalanceModal] = useState(false);
    ...
    const fetchStores = useCallback(async () => {
        ...
        try {
            ...
            if (response.data.httpStatusCode === 200) {
                ...
                showAlert(response.data.message || 'Şantiyenin Depo yüklenirken bir hata oluştu.', 'error');
            }
        } catch (e: any) {
            showAlert('Şantiyenin Depo yüklenirken bir hata oluştu.', 'error');
        } finally {
            setLoadingData(false);
        }
    }, [workhouseId, navigate]);
    ...
    }, [storesList, searchTerm, statusFilter, order, orderBy, startDate, endDate]);
    ...
    const exportStoresToExcel = (data: StoreType[], title: string) => {
        if (!data || data.length === 0) {
            showAlert('Excel oluşturulacak mağaza bulunamadı.', 'warning');
            return;
        }
    ...
    const handleDownload = (format: 'pdf' | 'excel', isFiltered: boolean) => {
        ...
        if (format === 'pdf') {
            exportStoresToPdf(dataToDownload, title, subtitle);
        } else {
            exportStoresToExcel(dataToDownload, title);
        }
    };
    ...
    const handleDownloadSingleStore = (format: 'pdf' | 'excel') => {
        ...
        const title = `Şantiyenin Depo Detayları: ${selectedStoreForDownload.name}`;

        if (format === 'pdf') {
            exportStoresToPdf(data, title);
        } else {
            exportStoresToExcel(data, title);
        }
    ...
    const handleOpenRowDownloadModal = (store: StoreType) => {
        setSelectedStoreForDownload(store);
        setOpenRowDownloadModal(true);
        handleCloseMenu();
    };
    const handleCloseRowDownloadModal = () => {
        setSelectedStoreForDownload(null);
    ...
    return (
        <>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                <Stack direction="row" spacing={1} justifyContent="flex-end">
                                    {editingId !== null ? (
                                        <>
                                            <CustomTooltip title={isTooltipGloballyEnabled ? "Seçili Şantiyenin Depo güncelleyin" : ""}>
                        ...
                        <Table aria-label="store table">
                            <TableHead sx={{ background: "rgb(149 147 125 / 65%)" }}>
                                <TableRow>
                                    ...
                                    <StyledTableCell>
                                        <TableSortLabel active={orderBy === 'code'} direction={orderBy === 'code' ? order : 'asc'} onClick={() => handleRequestSort('code')} sx={{ color: "#171c23" }}>
                                            <Typography variant="h6">Kod</Typography>
                                        </TableSortLabel>
                                    </StyledTableCell>
                                    <StyledTableCell>
                                        <TableSortLabel active={orderBy === 'address'} direction={orderBy === 'address' ? order : 'asc'} onClick={() => handleRequestSort('address')} sx={{ color: "#171c23" }}>
                                            <Typography variant="h6">Adres</Typography>
                                        </TableSortLabel>
                                    </StyledTableCell>
                                    <StyledTableCell><Typography variant="h6">Bölge</Typography></StyledTableCell>
                                    {!workhouseId && (<StyledTableCell><Typography variant="h6">Şantiye</Typography></StyledTableCell>)}
                                    <StyledTableCell>
                                        <TableSortLabel active={orderBy === 'createAt'} direction={orderBy === 'createAt' ? order : 'asc'} onClick={() => handleRequestSort('createAt')} sx={{ color: "#171c23" }}>
                            ...
                            <TableBody>
                                {paginatedStores.length > 0 ? (
                                        ...
                                        <TableRow key={row.id}>
                                            ...
                                            <StyledTableCell>
                                                <Chip
                                                    label={row.status}
                                                    sx={{
                                                        backgroundColor: row.recordStatus === 2 ? 'warning.light' : row.recordStatus === 1 ? 'error.light' : 'success.light',
                                                        color: row.recordStatus === 2 ? 'warning.main' : row.recordStatus === 1 ? 'error.main' : 'success.main',
                                                    }}
                                                />
                                            </StyledTableCell>
                                            <StyledTableCell>
                                                ...
                                                >
                                                    ...
                                                    <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu Şantiyenin Depo fişlerine git" : ""}>
                                                        <MuiMenuItem onClick={() => { if (selectedRowForMenu) { navigate(`/store/list-store-receipt/${selectedRowForMenu.id}`); handleCloseMenu(); } }}>
                                                            <ListItemIcon><IconReceipt width={18} /></ListItemIcon>
                                                            Şantiye Fişleri
                                                        </MuiMenuItem>
                                                    </CustomTooltip>

                                                    <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Depo'dan Merkeze Sevk İşlemi" : ""}>
                                                        <MuiMenuItem onClick={handleStoreDispatchToCenterClick}>
                                                            <ListItemIcon><IconArrowRight width={18} /></ListItemIcon>
                                                            Merkeze Sevk Et
                                                        </MuiMenuItem>
                                                    </CustomTooltip>
                                                    ...
                                                    <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu deponun envanterini görüntüle" : ""}>
                                                        <MuiMenuItem
                                                            onClick={() => {
                                                                if (selectedRowForMenu) {
                                                                    setSelectedStoreIdForModal(selectedRowForMenu.id);
                                                                    setStoreNameToBalance(selectedRowForMenu.name)
                                                                    setOpenViewBalanceModal(true);
                                                                    handleCloseMenu();
                                                                }
                                                            }}
                                                        >
                                                            <ListItemIcon><IconBoxSeam width={18} /></ListItemIcon> Envanteri Görüntüle
                                                        </MuiMenuItem>
                                                    </CustomTooltip>
                                                    ...
                                                    {hasEditPermission && (
                                                        <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu Şantiyenin Depo düzenle" : ""}>
                                                    ...
                                                    {hasDeletePermission && (
                                                        <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu Şantiyenin Depo sil" : ""}>
                                                    ...
                                                    {hasDownloadPermission && (
                                                        <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu Şantiyenin Depo indir" : ""}>
                                                            <MuiMenuItem onClick={() => handleOpenRowDownloadModal(row)}>
                                                                <ListItemIcon><IconFileDownload size={18} /></ListItemIcon>
                                                                Bu satırı indir
                                                            </MuiMenuItem>
                                                        </CustomTooltip>
                                                    )}
                                    ...
                                    <TableRow>
                                        <StyledTableCell colSpan={workhouseId ? 7 : 8} align="center">
            ...
            <ViewStoreBalanceModal
                open={openViewBalanceModal}
                onClose={() => {
                    setOpenViewBalanceModal(false);
                    setSelectedStoreIdForModal(null);
                }}
                storeId={selectedStoreIdForModal}
            ...
            <Dialog open={openRowDownloadModal} onClose={handleCloseRowDownloadModal} maxWidth="xs">
                ...
                <DialogContent>
                    <Stack direction="column" spacing={2} sx={{ mt: 2 }}>
                        <Button variant="contained" color="primary" startIcon={<IconFileText />}
                            onClick={() => handleDownloadSingleStore('pdf')}
                        >
                            PDF Olarak İndir
                        </Button>
                        <Button variant="contained" color="success" startIcon={<IconFileSpreadsheet />}
                            onClick={() => handleDownloadSingleStore('excel')}
                        >
                            Excel Olarak İndir
                        </Button>
                    </Stack>
