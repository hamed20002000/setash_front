...
import DeleteStoreDispatchToCenter from "./DeleteStoreDispatchToCenter";
...
interface StoreDispatchToCenterType {
    id: string;
    code: string;
    docDate: string;
    description: string,
...
const getStatusTextAndColor = (status: 0 | 1 | 2): { text: string, color: 'default' | 'success' | 'error' | 'warning' } => {
    switch (status) {
        case 0:
        ...
        default:
            return { text: 'Bilinmiyor', color: 'default' };
    }
};
...
const ListStoreDispatchToCenter = () => {
    ...
    const [dispatchList, setDispatchList] = useState<StoreDispatchToCenterType[]>([]);
    const [displayedDispatches, setDisplayedDispatches] = useState<StoreDispatchToCenterType[]>([]);
    ...
    const [selectedRowForMenu, setSelectedRowForMenu] = useState<StoreDispatchToCenterType | null>(null);
    ...
    const [selectedDispatchForDownload, setSelectedDispatchForDownload] = useState<StoreDispatchToCenterType | null>(null);
    ...
    const fetchDispatches = useCallback(async () => {
        ...
        try {
            const [driversRes, storesRes, dispatchesRes] = await Promise.all([
                ...
                axios.get<any>(server.baseurl + server.warehouse + `get-Store-dispatches-to-center/${Number(storeId)}`, { headers: { "Authorization": `Bearer ${authToken}` } }),
            ]);
            ...
            if (dispatchesRes.data?.httpStatusCode === 200) {
                const allDispatches: StoreDispatchToCenterType[] = dispatchesRes.data.data;
    ...
    const validateForm = (): boolean => {
        ...
        if (!selectedDestinationWarehouseId) {
            setDestinationWarehouseIdError(true);
            isValid = false;
        } else {
    ...
    const insertDispatch = async () => {
        ...
        try {
            const response = await axios.post(server.baseurl + server.warehouse + "create-store-dispatch-to-center", payload, { headers: { "Authorization": `Bearer ${authToken}`, "Content-Type": "application/json" } });
    ...
    const editDispatch = async () => {
        ...
        try {
            const response = await axios.put(server.baseurl + server.warehouse + "update-store-dispatch-to-center", payload, { headers: { "Authorization": `Bearer ${authToken}`, "Content-Type": "application/json" } });
            if (response.data.httpStatusCode === 200) {
    ...
    const handleStatusUpdateClick = (action: 'approve' | 'reject') => {
        if (selectedRowForMenu) {
            setStatusAction(action);
            setDispatchIdToAct(selectedRowForMenu.id);
            setDispatchCodeToAct(selectedRowForMenu.code);
            setOpenStatusModal(true);
            setStatusDescription(selectedRowForMenu.statusDescription || '');
        }
        handleCloseMenu();
    };

    const handleStatusUpdate = async () => {
        if (!dispatchIdToAct || !statusAction) return;

        if (statusAction === 'reject' && statusDescription.trim() === '') {
            setStatusDescriptionError(true);
            showAlert('Reddetme için açıklama zorunludur.', 'warning');
            return;
        }

        setLoadingButton(true);
        setStatusDescriptionError(false);

        const newStatus = statusAction === 'approve' ? 1 : 2;
        const payload: StatusUpdatePayload = {
            id: Number(dispatchIdToAct),
            status: newStatus,
            description: statusDescription
        };

        try {
            const response = await axios.put(server.baseurl + server.warehouse + "update-store-dispatch-to-center-status", payload, { headers: { "Authorization": `Bearer ${authToken}`, "Content-Type": "application/json" } });
            if (response.data.httpStatusCode === 200) {
                showAlert(`Sevk belgesi başarıyla ${statusAction === 'approve' ? 'onaylandı' : 'reddedildi'}!`, 'success');
    ...
    const exportDispatchesToExcel = (data: StoreDispatchToCenterType[], title: string) => {
    ...
    const handleDownload = (format: 'pdf' | 'excel', isFiltered: boolean) => {
        ...
        const title = isFiltered ? 'Filtrelenmiş Merkez Sevk Raporu' : 'Tüm Merkez Sevk Raporu';
    ...
    const handleOpenRowDownloadModal = (dispatch: StoreDispatchToCenterType) => {
            ...
            <Box sx={{ p: 3 }}>
                ...
                >
                    ...
                    >
                        {!isFormVisible && hasCreatePermission && (
                            <CustomTooltip title={isTooltipGloballyEnabled ? "Yeni Merkez Depo Sevk Belgesi kaydetmek için tıklayınız" : ""}>
                                <BlinkingButton
                                    variant="contained"
                                    color="primary"
                ...
                {((isFormVisible && !editingId && hasCreatePermission) || (editingId && hasEditPermission)) && (
                    <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
                        <Typography variant="h5" mb={2}>{editingId ? 'Merkez Sevk Belgesini Düzenle' : 'Yeni Merkez Sevk Belgesi'}</Typography>
                ...
                <BlankCard>
                    ...
                    {loadingData ? (
                        <Box display="flex" justifyContent="center" alignItems="center" height="200px">
                            <CircularProgress />
                            <Typography variant="h6" sx={{ ml: 2 }}>Merkez sevk belgeleri yükleniyor...</Typography>
                        </Box>
                    ) : (
                                                        ...
                                                        >
                                                            ...
                                                            {hasApproveRejectPermission && (
                                                                <>
                                                                    {row.status === 0 && (
                                                                        <>
                                                                            <MuiMenuItem onClick={() => handleStatusUpdateClick('approve')}>
                                                                                <ListItemIcon><IconCheck width={18} /></ListItemIcon>Onayla
                                                                            </MuiMenuItem>
                                                                            <MuiMenuItem onClick={() => handleStatusUpdateClick('reject')}>
            ...
            <Dialog open={openStatusModal} onClose={() => { setOpenStatusModal(false); setStatusDescriptionError(false); }} maxWidth="sm" fullWidth>
                ...
                <DialogContent dividers>
                    <CustomTextField
                        multiline
                        rows={4}
                        placeholder="İşlemle ilgili kısa bir açıklama giriniz."
                        value={statusDescription}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                            setStatusDescription(e.target.value);
                            if (statusDescriptionError && e.target.value.trim() !== '') setStatusDescriptionError(false);
                        }}
                        fullWidth
                        error={statusDescriptionError}
                        helperText={statusDescriptionError ? "Reddetme için açıklama zorunludur!" : ""}
                    />
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => { setOpenStatusModal(false); setStatusDescriptionError(false); }} color="secondary">
                        İptal
                    </Button>
                    <Button
                        ...
                        variant="contained"
                        disabled={loadingButton || (statusAction === 'reject' && statusDescription.trim() === '')}
                        startIcon={loadingButton ? <CircularProgress size={20} color="inherit" /> : null}
                    >
                        {loadingButton ? 'Bekleniyor...' : (statusAction === 'approve' ? 'Onayla ve Gönder' : 'Reddet ve Gönder')}
                    </Button>
                </DialogActions>
            </Dialog>
