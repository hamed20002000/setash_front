...
const ListRollCalls = () => {
            ...
            if (res.data.httpStatusCode === 200 && Array.isArray(res.data.data)) {
                const workhouseAssignments = res.data.data
                    ...
                    .map((r: any) => ({
                        ...
                        type: 1 as 1,
                        placeKind: 'WORKHOUSE',
                        placeName: r.workhouse?.name || 'Bilinmiyor',
        ...
        try {
            ...
            if (res.data.httpStatusCode === 200 && Array.isArray(res.data.data)) {
                const mappedData: RollCallType[] = res.data.data.map((r: any) => ({
                    ...
                    placeName: r.personnelWorkPlace.workhouse?.name || 'Bilinmiyor',
    ...
    const exportRollCallsToPdf = async (data: RollCallType[], isFiltered: boolean) => {
        ...
        const columns = [
            "ID", "Personel", "Mekan", "Tarih", "Başlangıç", "Bitiş", "Oluşturulma"
        ];
    ...
    const exportRollCallsToExcel = async (data: RollCallType[], isFiltered: boolean) => {
        ...
        const columns = [
            "ID", "Personel", "Mekan", "Tarih", "Başlangıç Saati", "Bitiş Saati", "Oluşturulma Tarihi"
        ];
        const headerTitle = isFiltered ? "Filtrelenmiş Yoklama Raporu" : "Tüm Yoklama Raporu";
                ...
                <Box sx={{ p: 2 }}>
                    <Grid container spacing={2} alignItems="center">
                        <Grid item xs={12} sm={6} md={8}>
                            <TextField
                                label="Personel, Mekan veya Unvan Ara"
                                variant="outlined"
                                fullWidth
                                value={searchTerm}
                        ...
                        <Table aria-label="roll call table">
                            <TableHead sx={{ background: "rgb(149 147 125 / 65%)" }}>
                                <TableRow>
                                    ...
                                    <StyledTableCell sx={{ color: "#171c23" }}>Mekan</StyledTableCell>
                                    <StyledTableCell sx={{ color: "#171c23" }}>
